...
public class DemoApplication {
    ...
    public String saveSim(String name, Date date, int score) {
        TycoonSimApplication savedSim = this.tycoonSimRepository.save(new TycoonSimApplication(name, date, score));
        return savedSim.toString();
    }

    @ShellMethod("Loads all sims")
    public String findAllSims() {
        Iterable<TycoonSimApplication> sims = this.tycoonSimRepository.findAll();
        return Lists.newArrayList(sims).toString();
    }

    @ShellMethod("Loads sims by name: find-by-name <name>")
    public String findByName(String name) {
        List<TycoonSimApplication> sims = this.tycoonSimRepository.findByName(name);
        return sims.toString();
    }

    @ShellMethod("Loads sims published by a score: find-by-score <score>")
    public String findByScore(int score) {
        List<TycoonSimApplication> sims = this.tycoonSimRepository.findByScore(score);
        return sims.toString();
    }

    @ShellMethod("Loads books by author and year: find-by-author-year <author> <year>")
    public String findByHighScore() {
        List<TycoonSimApplication> sims = this.tycoonSimRepository.findByHighScore();
        return sims.toString();
    }
